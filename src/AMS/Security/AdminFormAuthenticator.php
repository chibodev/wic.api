...
namespace App\AMS\Security;

use Exception;
use FOS\UserBundle\Model\UserInterface as FOSAdminUserInterface;
...
class AdminFormAuthenticator extends AbstractFormLoginAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider): ?UserInterface
    {
        ...
        /** @var FOSAdminUserInterface $admin */
        $admin = $this->userManager->findUserByEmail($credentials['email']);

        if (!$admin) {
            throw new CustomUserMessageAuthenticationException('Email could not be found.');
        }

        return $userProvider->loadUserByUsername($admin->getUsername());
    }
